...
public class ProyectoFinal {
    ...
    public static void main(String[] args) {
        ...
        if (tipoUsuario == 1) {
            ...
            if (clave.equals("1234")) { // puedes cambiar la clave
                esAdmin = true;
            } else {
        ...
        do {
            ...
            if (opcionAdmin == 1) {
                ...
                String nuevo = JOptionPane.showInputDialog("Nombre del nuevo producto:");

                int precioNuevo;
            try {
                precioNuevo = Integer.parseInt(JOptionPane.showInputDialog("Precio:"));
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, "Precio inválido");
                continue; 
            }

            String[] nuevosNombres = new String[nombres[categoria].length + 1];
            int[] nuevosPrecios = new int[precios[categoria].length + 1];
            int[] nuevasCantidades = new int[cantidades[categoria].length + 1];

            for (int i = 0; i < nombres[categoria].length; i++) {
                nuevosNombres[i] = nombres[categoria][i];
                nuevosPrecios[i] = precios[categoria][i];
                nuevasCantidades[i] = cantidades[categoria][i];
            }

            nuevosNombres[nombres[categoria].length] = nuevo;
            nuevosPrecios[precios[categoria].length] = precioNuevo;
            nuevasCantidades[cantidades[categoria].length] = 0;

            nombres[categoria] = nuevosNombres;
            precios[categoria] = nuevosPrecios;
            cantidades[categoria] = nuevasCantidades;
